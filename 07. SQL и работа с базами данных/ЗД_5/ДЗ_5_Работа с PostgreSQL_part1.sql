...
select  p.customer_id, p.payment_id, p.payment_date, row_number() over(order by p.payment_date::date)
from payment p

select  p.customer_id, p.payment_id, p.payment_date, row_number() over(partition by p.customer_id order by p.payment_date::date)
from payment p

select p.customer_id, p.payment_id, p.payment_date, sum(p.amount) over (partition by p.customer_id order by p.payment_date::date, p.amount)
from payment p
...
select p.customer_id, p.payment_id, p.payment_date, p.amount, 
	   lag(p.amount,1,0) over(partition by p.customer_id order by p.payment_date::date) last_amount
from payment p 
...
select p.customer_id, p.payment_id, p.payment_date, p.amount,
	   p.amount - lead(p.amount,1,0) over(partition by p.customer_id order by p.payment_date::date) difference
from payment p 
