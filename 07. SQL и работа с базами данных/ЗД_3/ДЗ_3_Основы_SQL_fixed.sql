...
select t.store_id "ID магазина", t.count "Количество покупателей", c2.city Город, 
	   concat_ws(' ', s.last_name, s.first_name) "Имя сотрудника" 
from (
	select c.store_id, count(c.store_id)  
	from customer c 
	group by c.store_id
	having count(c.store_id) > 300) as t
join address a on a.address_id = t.store_id	
join city c2 on c2.city_id = a.city_id 
...
from (
	...
	order by count(i.film_id) desc -- или можно сделать после соединения! 
	limit 5) as t
...
select concat_ws(' ', c.last_name, c.first_name) "ФИО", t.count "Количество фильмов", 
	   t.ceil "Общая стоимость платежей", t.min "МИН. стоимость платежа", 
	   t.max "МАКС. стоимость платежа"
from (
	select r.customer_id, count(i.film_id), ceil(sum(p.amount)), min(p.amount), max(p.amount)
	from payment p 
	join rental r on r.rental_id = p.rental_id          		
	join inventory i on i.inventory_id = r.inventory_id 
	group by r.customer_id) as t
join customer c on c.customer_id = t.customer_id
--where concat_ws(' ', c.last_name, c.first_name) = 'WILES JON' -- CROUSE ALBERT, COLLAZO TOMMY, WILES JON, TUBBS DUANE, WATSON THERESA 
																--(<ДЛЯ ПРОВЕРКИ ПО СКРИНШОТУ>)	
